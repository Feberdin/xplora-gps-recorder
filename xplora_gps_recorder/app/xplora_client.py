...
# Xplora does not publish a stable public API contract, so we intentionally keep
# the minimal GraphQL contract in one place. Sensitive Open API credentials come
# from configuration to keep them out of git history and runtime logs.
DEFAULT_USER_AGENT = (
...
class XploraClient:
    ...
    def _build_headers(self, *, use_auth: bool) -> dict[str, str]:
        ...
        if use_auth:
            ...
            authorization = (
                f"Open {self.settings.xplora_open_api_key}:{self.settings.xplora_open_api_secret.get_secret_value()}"
            )
